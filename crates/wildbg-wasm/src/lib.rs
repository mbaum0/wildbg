...
use logic::cube::{CubeState, MatchState};
...
impl Wildbg {
    ...
    pub fn cube_info_internal(&self, pips: Vec<i8>) -> Result<CubeDecision, String> {
        let position = position_from_vec(pips)?;
        // This fork's cube API models cube ownership and the match score, which
        // upstream's single-argument version did not. A centered cube in a money
        // game reproduces the behaviour this entry point had upstream.
        let cube = self
            .api
            .cube_info(&position, CubeState::default(), MatchState::Money);
        Ok(CubeDecision {
